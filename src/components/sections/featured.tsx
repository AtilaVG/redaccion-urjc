"use client";

import Link from "next/link";
import Image from "next/image";
import { ArrowUpRight, FileText, Users } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Reveal } from "@/components/animations/reveal";
import { SectionHeading } from "./section-heading";
import { getFeatured, getCollection } from "@/lib/collections";
import { formatDate } from "@/lib/utils";

export function Featured() {
  const [lead, ...rest] = getFeatured();
  if (!lead) return null;
  const leadCol = getCollection(lead.collection);

  return (
    <section className="relative mx-auto max-w-7xl px-6 py-24 sm:py-32">
      <SectionHeading
        kicker="Destacado"
        title="La selección de nuestro comité editorial"
        description="Publicaciones recientes con especial impacto académico y social."
        className="mb-14"
      />

      <div className="grid gap-6 lg:grid-cols-2">
        <Reveal>
          <Link
            href={`/publicaciones/${lead.slug}`}
            data-cursor="hover"
            className="group glass relative flex h-full min-h-[420px] flex-col justify-end overflow-hidden rounded-2xl p-7"
          >
            <Image
              src={lead.cover}
              alt={lead.title}
              fill
              sizes="(max-width: 1024px) 100vw, 50vw"
              priority
              className="object-cover transition-transform duration-[1.2s] ease-out group-hover:scale-105"
            />
            <div className="from-void via-void/70 to-void/10 absolute inset-0 bg-gradient-to-t" />
            <div className="relative z-10">
              <div className="mb-4 flex gap-2">
                <Badge variant="violet">{leadCol.name}</Badge>
                <Badge variant="glass">{lead.type}</Badge>
                {lead.isNew && <Badge variant="live">Novedad</Badge>}
              </div>
              <h3 className="font-display max-w-xl text-3xl leading-tight font-bold text-balance text-white sm:text-4xl">
                {lead.title}
              </h3>
              <p className="mt-3 max-w-lg text-pretty text-white/70">
                {lead.excerpt}
              </p>
              <div className="mt-6 flex flex-wrap items-center gap-4 text-sm text-white/60">
                <span className="flex items-center gap-1.5">
                  <Users className="size-4" /> {lead.authors.join(", ")}
                </span>
                <span>{formatDate(lead.date)}</span>
                {lead.pages && (
                  <span className="flex items-center gap-1">
                    <FileText className="size-3.5" /> {lead.pages} p.
                  </span>
                )}
              </div>
            </div>
            <span className="glass-strong absolute top-6 right-6 z-10 flex size-11 items-center justify-center rounded-full text-white transition-transform duration-300 group-hover:rotate-45">
              <ArrowUpRight className="size-5" />
            </span>
          </Link>
        </Reveal>

        <div className="flex flex-col gap-6">
          {rest.map((pub, i) => {
            const col = getCollection(pub.collection);
            return (
              <Reveal key={pub.slug} i={i + 1}>
                <Link
                  href={`/publicaciones/${pub.slug}`}
                  data-cursor="hover"
                  className="group glass hover:bg-foreground/5 grid grid-cols-[140px_1fr] gap-4 overflow-hidden rounded-2xl p-3 transition-colors sm:grid-cols-[180px_1fr]"
                >
                  <div className="relative aspect-[4/3] overflow-hidden rounded-xl">
                    <Image
                      src={pub.cover}
                      alt={pub.title}
                      fill
                      sizes="180px"
                      className="object-cover transition-transform duration-700 group-hover:scale-110"
                    />
                  </div>
                  <div className="flex flex-col justify-center pr-3">
                    <Badge variant="cyan" className="w-fit">
                      {col.name}
                    </Badge>
                    <h4 className="group-hover:text-gradient font-display mt-2 text-lg leading-snug font-semibold text-balance transition-colors">
                      {pub.title}
                    </h4>
                    <span className="text-muted-foreground mt-2 flex items-center gap-1 text-xs">
                      {pub.type} · {formatDate(pub.date)}
                    </span>
                  </div>
                </Link>
              </Reveal>
            );
          })}
        </div>
      </div>
    </section>
  );
}
