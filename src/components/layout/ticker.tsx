...
import { getLatest } from "@/lib/collections";

export function Ticker() {
  const items = getLatest().map((p) => p.title);
  const row = [...items, ...items];

  return (
    <div className="border-border/60 bg-card/40 relative z-20 flex items-center gap-4 border-y py-3 backdrop-blur-md">
      <div className="text-garnet flex shrink-0 items-center gap-2 pl-4 text-xs font-semibold tracking-widest uppercase">
